...
## 0.36.0
Finishes the redesign: the surfaces that were still browser-native, keyboard and screen-reader access, and touch support for the gestures that only worked with a mouse.
- Real sign-in screen instead of the browser's `prompt()`. That prompt was the first thing the Cockpit ever showed on a phone, and it could neither explain where the token comes from nor say that the one you typed was wrong
- Wrong tokens are now detectable. The protocol has no auth-failure event — the engine just closes the socket — so the client used to retry forever, leaving you in front of a dot that never turned green. It now separates the two cases: a socket that never opens means the engine is unreachable and retrying is right; a socket that opens, sends auth and is closed without `auth_ok` twice means the token, and the sign-in screen comes back with an explicit message
- `window.confirm` replaced in the System and ToDoMio panels. The native one cannot label its action ("OK" does not say that a process is about to be killed), cannot mark a destructive choice, and on Android appears at the top of the screen — as far from the thumb as possible, on a button that confirms something irreversible
- Project rows and session tabs were `<div onClick>`: unreachable by keyboard and never announced as controls. They are now real buttons with the tab pattern — roving tabindex, arrows, Home/End with manual activation. Both already contained nested buttons, which is invalid HTML, so each row grew a wrapper that also carries the highlight; without it the active pill stopped short of the pencil and close controls
- ARIA roles, Esc, and focus that enters and returns for the floating-panel shell (eight panels at once), the permission prompt and the command palette; `aria-label` on icon-only buttons, which relied on `title=` and therefore said nothing at all on touch
- Window dragging and rail resizing converted from mouse to pointer events, so they work with a finger. This surfaced a real leak: pointer capture can be lost without `pointerup` or `pointercancel` — an element hidden mid-gesture only emits `lostpointercapture` — and the `pointermove` listener was never removed. For the rail handle, which stays mounted for the life of the app and becomes `display: none` below 768px, every gesture after a mid-drag resize piled up another listener with stale closures fighting over the width
- File navigator context menu opens on long press, plus a visible overflow button on coarse pointers — a long press nobody discovers leaves the menu exactly as unreachable as it was
- Disabled primary buttons stop looking pressable app-wide: `.mini.primary` was declared after `.mini:disabled`, so the coral fill won and every disabled primary action stayed inviting
