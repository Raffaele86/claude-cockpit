 ...
 *  Resta aperto mentre si regolano più valori; il cambio provider in CLI lo chiude (relaunch). */
          ...
          <div className="smenu-row">
            ...
            <div className="provider-toggle">
              {providers.map((p) => (
                <button
                  key={p}
                  className={ctl.curProv === p ? 'prov on' : 'prov'}
                  onClick={() => {
                    ctl.setProv(p);
                    if (ctl.cli) setOpen(false); // relaunch del terminale
                  }}
                >
          ...
          <div className="smenu-row">
            ...
            <div className="provider-toggle">
              {ctl.modes.map((m) => (
                <button
                  key={m.key}
                  className={ctl.curMode === m.key ? 'prov on' : 'prov'}
                  onClick={() => {
                    ctl.setPerm(m.key);
                    if (ctl.cli) setOpen(false); // relaunch del terminale
                  }}
